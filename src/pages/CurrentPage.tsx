...
const Page = (url?: any) => {
  ...
  async function handleClickShowMore(element: any, props: any, parentId: any) {
      ...
      let temp = (
        <div className="comment" key={response.data.id}>
          ...
          <div
            ...
            className={`comment-${response.data.id}`}
          ></div>
        </div>
      );
      arr.push(temp);
      ...
      <section className="personal-page">
        ...
        {news.url === "" ? (
          ...
          <div className="only-news">
            ...
            <p>
              title: <span className="only-news__text">{news.title}</span>
            </p>
            <p>
              date:
...
async function News(
  props: number,
  setNavigateTo: React.Dispatch<React.SetStateAction<Boolean>>,
  setNews: any
) {
